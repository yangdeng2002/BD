...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], stride=1, dilate=replace_stride_with_dilation[1], filter_size=filter_size)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1, dilate=replace_stride_with_dilation[2], filter_size=filter_size)
...
def resnet50(pretrained=False, filter_size=4, pool_only=True, _force_nonfinetuned=False, **kwargs):
    ...
    if pretrained:
        if(filter_size==4):
            state_dict=torch.load('./weight/bd_res50.pth.tar')['state_dict']
            new_state_dict = {}
